...
| 7 | Sello de fuente | Regla del repo "institución sí, puente no": el sello nombra **CBOT · NYMEX · COMEX · ICE** (+ FX para USD/BRL), NUNCA el proveedor técnico. Y es honesto con el atraso: **"futuros demorados 10 min"** (SPY y USD/BRL van en tiempo real — medido, ver §3.b; stamps honestos, auditoría 07/07). |
...
- **Delay medido (con mercado abierto, 20/07): 10 min EXACTOS en todos los futuros
  (CBOT/NYMEX/COMEX) y el DXY; SPY y USD/BRL en TIEMPO REAL (0 min)** — ver §3.b, se investigó si
  alguna fuente del catálogo gauss lo baja: no. (Ojo trampa: medir el delay en sesión nocturna finita
  da números inflados — `regularMarketTime` es el último trade, no el atraso del feed.)
...
### 3.b ¿Alguna fuente de gauss con menos delay? — investigado 20/07/2026: **NO**

Pedido de Lautaro: ver si otra fuente del catálogo baja el delay. Se midió con requests reales
(mercado abierto, `tradeTime`/`regularMarketTime` vs reloj) y se barrió el catálogo entero:

| Fuente (skill gauss) | Delay medido/documentado | Veredicto |
|---|---|---|
| **Yahoo Finance** (elegida) | **Futuros + DXY: 10 min exactos · SPY: 0 min · USD/BRL: 0 min** (medido) | Se queda: única con batch de 11 + tiempo real en SPY/BRL |
| **Barchart** (fallback) | **10,0–10,1 min** (medido: ZSX26 y CLU26, flow cookie XSRF + header `referer` obligatorio en `/quotes/get`) | Mismo delay, más frágil — sigue de fallback |
| **Investing** | HTTP **403** (Cloudflare, IP datacenter) + declara 15-20 min | Descartada dos veces |
| TradingView · Google Finance · MarketWatch | Futuros CME demorados (mismo piso regulatorio) | No mejoran nada |
| Alpaca Data | Tiempo real IEX **solo acciones** (con API key) — NO tiene futuros | Solo mejoraría SPY… que en Yahoo ya es tiempo real |
| data912 (refresh 20 s) · Primary/MAE/BYMA/BCRA | Argentina/local — **no tienen CBOT ni macro global** | No aplican |
| CBOE Data | Índices/opciones CBOE (VIX/SPX), no granos CBOT | No aplica |

**Por qué nadie baja de 10 min**: es el **piso de licencia de CME/ICE** para redistribución
gratuita — cualquier fuente gratis de futuros de Chicago va a dar ≥10 min, siempre. Menos delay =
feed licenciado pago (CME real-time, un broker de futuros) → fuera del alcance de este monitor;
si algún día se quiere, es una decisión de costo aparte. Bonus del hallazgo: SPY y USD/BRL ya
salen en tiempo real por la misma fuente sin hacer nada.
...
| Delay de 10 min en futuros (piso de licencia CME/ICE — §3.b: ninguna fuente gratis lo baja) | Sello honesto "futuros demorados 10 min" + hora del dato visible. NO venderlo como tiempo real. |
