...
- **Fix `num()` en el cargador y el parser**: el export real trae artefactos de float con punto
  ...
  con grupos de 3 dígitos exactos. Con la próxima re-subida del CSV por el uploader esas 477 filas
  quedan corregidas en la base.
...
- **Aplicar la migración `20260720120000_admin_carga_compras.sql`** (orquestador, por MCP
  `execute_sql`/`apply_migration`). Hasta entonces el uploader previsualiza OK pero el paso 2
  muestra el error de PostgREST ("function ... does not exist").
- **Que Lautaro pruebe el uploader logueado** (elegir archivo → Previsualizar → Confirmar) y de
