...
fallando en sus dos últimas corridas programadas (16/07 y 17/07) por timeout de conexión**, y nadie
lo notó en 5 días y varias sesiones (incluida la auditoría E1 del 21/07) — el dato lleva 8 días sin
refrescar. Un segundo hallazgo cierra una promesa que en realidad ya estaba resuelta desde hace 9
días: el probe de PAS (BCBA) SÍ corrió el 12/07 y su log ya dice todo lo que hacía falta saber (HTTP
403, Cloudflare bloquea también las IPs de GitHub Actions) — nadie lo leyó hasta ahora. Aparte de
eso: dos contradicciones concretas entre `PLAN_PUERTOS.md` y la realidad (quedó sin tocar desde el
18/07 pese a que sus Fases 2-4 se hicieron los días siguientes), la confirmación de que las tres
listas de pendientes efectivamente divergen (con `PLAN_BACKLOG.md` ya escrito el 21/07 como la
solución, solo falta que las listas viejas apunten ahí), y 7 ramas remotas con PR ya mergeado que
nadie borró todavía.
...
|---|---|---|---|
| Validar PAS (BCBA) desde Actions | PR #23/#24, sesión 12/07-estimaciones-argentina, semilla de E1/E2/E5/E6 en `PLAN_AUDITORIA.md` | **Ya resuelto desde el 12/07**, nunca cerrado en los docs (hallazgo #2) | Cerrar formalmente, ver hallazgo #2 |
| DEA-SAGyP: crons mantienen el dato al día | PR #21/#23, "los crons mantienen solo" repetido en varios docs | **Roto** desde el 16/07 (hallazgo #1) | Ver hallazgo #1 |
...
1. **DEA-SAGyP (hallazgo #1):** ¿querés que re-corra el dispatch ahora mismo para ver si el timeout
   fue transitorio, o preferís que quede documentado y lo resuelva la próxima sesión de infraestructura
   (E5)? Si vuelve a fallar, la fuente en `datosestimaciones.magyp.gob.ar` puede estar empezando a
   bloquear las IPs de GitHub Actions igual que hace `bolsadecereales.com` (BCBA) — ahí la Edge
   Function de Supabase (mismo patrón que `lineup-ingest`) sería la mitigación, no un parche del script.
2. **PAS (hallazgo #2):** ¿confirmás que activemos el respaldo por mail (suscripción tuya al informe
   PAS) y demos de baja definitivamente la idea de automatizarlo vía scraping? El probe ya contestó
   que no hay IP disponible que lo destrabe.
3. **Uploader de `/admin/datos`:** ¿ya lo probaste logueado (subir el export de Agrochat →
   previsualizar → confirmar)? Si no, es el único paso que falta para dar por cerrado el ítem
   8/9 del backlog del todo.
4. **Lista única de pendientes:** ¿das el OK para que `CONTEXTO.md` deje de tener su propia sección
   «Pendientes» y en su lugar apunte a `ESTADO.md` («Plan RF AGRO») + `PLAN_BACKLOG.md` (que ya hace
   el mapeo completo)? Es solo reordenar texto, cero cambios de código.
